...
The primary user-facing way to talk with the Personal Agent, available as a full-screen interactive Terminal Conversation in the first version; running `myclaw` without arguments enters it. Each valid Terminal Conversation start prepares a new Conversation Session by default; a session is not persisted if the application exits before any user message, and one process reuses its session across inputs. Only built-in slash commands are handled by the Management Port; other inputs beginning with `/` are treated as ordinary user messages and sent to the model. Users can run `/resume` inside Terminal Conversation to choose and switch to a prior session from the current Workspace using an interactive picker. The previous session is retained if it has messages and can be discarded if empty.
