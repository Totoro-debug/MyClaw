...
**Agent Home**:
The fixed `~/.myclaw/` location that stores global Personal Agent data such as User Configuration, Conversation Summary, Long-term Memory, and Runtime Log files. Durable state replacements under Agent Home must be atomic so cancellation cannot corrupt existing files; Runtime Log append is an explicit diagnostic exception governed by its own whole-record, rotation, and cross-process coordination rules. First startup creates the base directories `memory/` and `sessions/`, Long-term Memory is initialized at startup if missing, Scheduled Work is stored in the root `scheduled-work.json`, consolidation recovery journals are created on demand under `memory/pending-consolidations/`, Runtime Log state is created on demand under `logs/`, and other runtime files are created on demand.
_Avoid_: Project workspace, session directory, install directory, configurable data root

**Workspace**:
The current user-selected working directory for a Personal Agent interaction, identified by its normalized absolute path. Its Agent Home directory name is a lowercase path slug formed by joining path segments with `-` and replacing existing `-` characters inside segments with `_`, such as `D:\desktop\project\Demo-one` becoming `d-desktop-project-demo_one`. Conversation Sessions are grouped by Workspace under Agent Home; file capabilities are bounded to the Workspace plus explicitly allowed Agent Home paths, while Shell requires a Workspace cwd and only runs the fixed safe read-only command allowlist. The Tool architecture in #38 temporarily deviates from ADR-0003 by refusing every non-allowlisted Shell command because user confirmation is not implemented in the current contract; Workspace cwd validation remains neither an operating-system filesystem sandbox nor a network sandbox.
