...
Agent Runner is the sole bounded ReAct implementation. Foreground and User Schedule work share the current Agent Loop's Runner and generation-owned Tool instances, but each uses an independent Agent Run Gateway view with isolated Catalog and Tool Activation state. Dream is a single-completion responsibility that calls Model Router directly and applies only returned Long-term Memory edits, as recorded in [ADR-0024](0024-use-one-shot-dream-model-request.md). Context construction follows [ADR-0018](0018-centralize-model-request-context-construction.md).
