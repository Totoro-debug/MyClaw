...
User Schedule execution shares the current Agent Loop's Gateway and Runner identities but uses its own Session, context, cancellation, externalizer, and Session Log. It has no interactive confirmation callback or foreground output callback. Dream owns a separate Runner and restricted Gateway.
