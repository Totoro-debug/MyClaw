...
| Shell trust and lifecycle | Automatic Git could run a Workspace/PATH shadow or repository hooks/filters; a nonzero process result exposed raw output; cleanup ownership could be lost after transient stop failures or cancellation during spawn. | Automatic Git is pinned to a trusted executable and hardened environment; ambiguous repositories downgrade to ASK/refusal; nonzero output is normalized; cleanup remains owned and retryable until the process tree is reaped. | [test_security_shell.py](../tests/test_security_shell.py), [test_shell_process.py](../tests/tools/shell/test_shell_process.py) |
...
| Disk and publication failure | Initial, assistant, Tool, repair, model-error, and metadata publication can fail before write, after a durable effect, or while the durable state cannot be reloaded. | Exact durable messages are reconciled before deciding whether to retry, repair, commit, or discard; no successful terminal event follows a required persistence failure; ordinary appended JSONL remains complete when metadata rewrite fails. | [test_security_fault_injection.py](../tests/test_security_fault_injection.py), [test_session_store.py](../tests/sessions/test_session_store.py), [test_scheduled_work_execution.py](../tests/scheduling/test_scheduled_work_execution.py) |
