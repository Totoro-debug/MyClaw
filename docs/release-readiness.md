...
| Skills | Each AgentLoop captures one Runtime Generation Skill Snapshot for foreground metadata discovery and composition-side manual/always projection, while autonomous `read_file` calls retain live filesystem semantics; raw-input Session persistence and presentation-only shared completion do not change the fixed Tool Catalog. | `tests/skills/test_catalog.py`, `tests/agent/test_fixed_catalog.py`, `tests/agent/test_context.py`, `tests/agent/test_loop.py`, `tests/terminal/test_conversation.py`, `tests/tools/core/test_file_tools.py` |
