...
MCP Tool lifecycle and the configured MCP extension are defined by ADR-0020; references
below to the fixed Tool Catalog describe the ten Built-in Tools and remain valid for that
baseline.
...
| Tools and safety | The fixed ten-Tool Built-in Catalog and configured MCP Tool Snapshot execute only through `ToolGateway.call()` and the final BaseTool preparation pipeline. Exact-call confirmation protects external paths and bounded Exec/Web risks without claiming an OS sandbox; trusted MCP Tools follow ADR-0020. | `tests/tools/test_fixed_tool_gateway.py`, `tests/tools/core/test_file_tools.py`, `tests/tools/core/test_exec.py`, `tests/tools/core/test_web_fetch.py`, `tests/test_cli_mcp_lifecycle.py` |
...
- The fixed Built-in Tool Catalog contract remains covered by `tests/tools/test_fixed_tool_gateway.py::test_fixed_catalog_order_and_detached_definitions`, which asserts exactly ten Built-in Tool schemas; configured MCP snapshots are covered by `tests/test_cli_mcp_lifecycle.py` and `tests/tools/test_mcp_runtime.py`.
