...
- REPL 支持 streaming、工具确认、后台 Memory Task、Scheduled Work、session resume 和管理 slash commands。
...
26. 作为个人用户，我想文件新建和编辑需要确认，所以 Agent 不会静默修改项目。
...
32. 作为个人用户，我想创建自然语言 Scheduled Work，所以 Agent 能按计划执行任务。
33. 作为个人用户，我想创建 Scheduled Work 前进行确认，所以不会误创建持久化任务。
...
45. 作为开发者，我想 Tool Gateway 统一校验参数、权限和结果，所以工具行为保持一致。
...
- memory 模型通过受限 Tool Gateway 读取 Long-term Memory，并且只能编辑 `~/.myclaw/memory/memory.md`。
...
- Tool Gateway 负责 tool resolution、parameter validation、内置 Permission Policy、sandboxing、audit semantics 和 result normalization。
- Tool calls 不自动 retry；失败直接返回 tool result。
- Tool Gateway 不序列化前台和后台工具调用。
- File read/list/search 默认 allow。
- Workspace 内新建文件和编辑已有文件默认 ask。
- 主 Agent 不可编辑 config、memory、session、summary、cursor、Scheduled Work 等 Agent Home 内部文件。
- File access 越过 Workspace 和允许的 Agent Home 范围时直接拒绝，不升级为 ask。
- Shell 与 Web 可由配置启用/禁用；开关同时适用于前台 chat 和 Scheduled Work。
- Shell cwd 可指定，但必须位于 Workspace 内。
- Shell timeout 由模型请求，代码强制限制在 60–600 秒。
- Shell 自动 allowlist 只接受 `pwd`、`git status`、`git status --short`、`git diff --stat`、`git diff --name-only` 五种精确命令形状，用户不能扩展；其他语法有效且 cwd 合法的命令在前台一律 ask，在后台一律拒绝。
- 首版 Shell 强制 cwd 位于 Workspace，但不宣称提供 OS 级文件系统或网络隔离；前台用户确认的命令仍可能显式引用 Workspace 外路径。
...
- 需要用户确认的前台工具调用阻塞当前 turn。
- 接受确认不写单独历史；拒绝写对应 tool result。
- 后台任务遇到 ask 时按拒绝处理。

### Tool Artifacts

- 所有工具结果超过 `max_tool_result_chars = 50000` 时统一外部化。
- Artifact 路径为 `~/.myclaw/sessions/<workspace_slug>/artifacts/<session_id>/<tool_call_id>.txt`。
- Artifact 保存原始工具结果，必须原子写入。
- Session tool message 保存 artifact 引用和前 2000 个 Unicode code points 的截断 preview。
...
- 创建 Scheduled Work 是持久副作用，必须用户确认。
...
- Scheduled Work 创建确认和后台 ask 按拒绝处理。
- Tool Artifact 的阈值、路径、原始内容、preview 和原子写入。
- Tool calls 失败时不自动 retry。
